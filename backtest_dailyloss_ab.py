"""A/B sweep: DAILY_LOSS ∈ {800, 1200, 1600, 2000} on production config.

Production profile (matches run_prod.py): 3k vol bars, single-lot,
DAILY_PROF=0 (no profit cap), C_OPEN_COOL=36, NEWS_FILTER=1, M4 on.
Only DAILY_LOSS varies. Apex live = $800 (one M4 stop = one trade per day).

Hypothesis: raising daily-loss cap lets the strategy take 2nd/3rd
attempts after a stop-out instead of locking the day. Watch for: more
trades, WR holding, MaxDD blow-out is the obvious risk.

Per [[project_daily_loss_eq_one_stop]]: $800 = one 40pt M4 stop. Raising to
$1,200 ≈ 1.5 stops; $1,600 ≈ 2 stops; $2,000 ≈ 2.5 stops.

Usage: python backtest_dailyloss_ab.py <path.scid>
"""
import csv
import sys
...
SCENARIOS = {
    "loss800_live":  {**_PROD, "DAILY_LOSS": 800.0},
    "loss1200":      {**_PROD, "DAILY_LOSS": 1200.0},
    "loss1600":      {**_PROD, "DAILY_LOSS": 1600.0},
    "loss2000":      {**_PROD, "DAILY_LOSS": 2000.0},
}
BASELINE = "loss800_live"
...
def run_scenario(name, overrides):
    ...
    stem = SCID.stem
    out = BASE / f"IOF_NQ_dailyloss_{stem}_{name}.csv"
...
def main():
    print(f"\nA/B sweep on {SCID.name}: DAILY_LOSS {{800, 1200, 1600, 2000}}")
    results = {name: summarize(run_scenario(name, ov)) for name, ov in SCENARIOS.items()}

    print("\n" + "=" * 90)
    print(f"COMPARISON: daily-loss-cap sweep on {SCID.stem}  (prod config)")
    print("=" * 90)
