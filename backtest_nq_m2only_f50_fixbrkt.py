"""
M2-ALONE, floor 50, fixed 75/125pt bracket — cross-contract NQ.

M2 (mode 1) uses the /15 quality grid: q100 = (finalScore*100)//15, so floor 50
=> finalScore>=8 (score 7 -> q46 rejected; score 8 -> q53 admitted). Much
stricter than M4's *10 grid. Everything else identical to the M4-only harnesses
(fixed 300t/75pt stop + 500t/125pt target, MT=1, news ON, native 3000-vol,
1 NQ lot @ $20/pt, pure tick bracket).

Prior (different config — M2-only ATR exits, 6-contract): floor 50 t=-2.82,
floor 46 t=-2.63, both significantly NEGATIVE; M2 score gradient inverted
(tighter floor = worse). See project_m2_only_strategy_sweep / project_m2_disable_validated.
"""
...
CONTRACTS = {
    "NQU26": "F.US.ENQU26.scid",
    "NQZ25": "F.US.ENQZ25.scid",
    "NQM5":  "F.US.ENQM25.scid",
    "NQH6":  "F.US.ENQH26.scid",
}
NQ_LOTS = 1
...
_PANEL = dict(
    ...
    QUAL_FLOOR=50,                      # M2 (/15 grid) => finalScore>=8
    DISABLE_MODES={0, 2, 3, 4, 5, 6, 7},  # leave ONLY M2 (mode 1)
...
def run(label, fname):
    ...
    trades = bt.run()
    backtest.write_csv(trades, BASE / f"IOF_NQ_m2f50_{label}.csv")
    return trades
...
def main():
    ...
    print("\n" + "=" * 84)
    print(" M2-ONLY  floor 50  fixed 75/125pt bracket  (MT=1, 1 NQ lot, native 3000-vol)")
    print("=" * 84)
