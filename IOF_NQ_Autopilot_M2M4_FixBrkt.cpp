...
SCDLLName("IOF_NQ_Autopilot_M2M4_FixBrkt")

// ============================================================================
//  VARIANT BUILD — v12.38-M2M4FB — NOT THE VALIDATED LIVE CONFIG.
// ============================================================================
//  Requested config (2026-07-30). Deltas vs production v12.38:
//
//    1. MODES        : M2 + M4 only. M1, M3, M6, M8 hard-masked (M5/M7 were
//                      already inactive). See C_VAR_ENABLE_M* below.
//    2. QUALITY FLOOR: 40 for every enabled mode (prod = 50; M8 floor is dead
//                      code here since M8 is masked off).
//    3. MAX TRADES   : 3 / day (prod = 1).
//    4. BRACKET      : FIXED $500 stop / $500 target per contract, replacing
//                      the ATR stop + T1/T2 + breakeven + ATR-trail machinery.
//                      Distances are derived from PtVal so the build is
//                      symbol-correct: NQ ($20/pt) -> 25.00 pts,
//                      MNQ ($2/pt) -> 250.00 pts. 1R risk:reward = 1:1.
//                      Early-scratch (v12.37), T1 partial, breakeven-shift and
//                      the ATR trail are all bypassed under the fixed bracket —
//                      a fixed target and a trail are mutually exclusive.
//    5. DAILY LOSS   : $1,600 (prod = $800). REQUIRED for #3 to be reachable:
//                      the session-DD gate blocks new entries once drawdown
//                      exceeds 0.8 * DAILY_LOSS, so $800 would lock the day
//                      after the 2nd $500 stop and MAX_TRADES=3 could never
//                      bind. $1,600 = 3 stops + $100 slippage headroom.
//
//  STILL ACTIVE (unchanged risk controls that can cut the day below 3 trades):
//    - C_MAX_LOSSES = 2 consecutive losses -> no further entries that session.
//    - C_POST_STOP_COOLDOWN = 10 bars, same-direction only.
//    - Late-entry gate (v12.36): no new SETUPs at/after 15:00 ET.
//    - News filter, regime filter, auto-disable, RM floor: all as production.
//
//  PRIOR EVIDENCE AGAINST THIS CONFIG — read before risking money:
//    - Quality floor < 50 has been falsified 4x cross-contract (qf33, qf40,
//      per-mode floor profile, M4 floor 40). Floor 50 survived every A/B.
//    - M2 standalone re-tested significantly negative (f50 t = -2.82) and the
//      quality gradient came out INVERTED — lower floor made M2 worse.
//    - M2+M4 fixed-bracket was tested on MNQ and falsified (-$4,666, t=-3.41).
//      This build is the NQ arm of that same idea; it has not been run.
//    This variant is a separate DLL and does NOT touch the live study. Run it
//    through a 2+ contract A/B before it goes anywhere near a funded account.
// ============================================================================
...
// [VARIANT M2M4FB] Floor 50 -> 40 as requested. This is the setting the
//          v12.30 note above says cannot ship on the evidence available; it is
//          here because this build is an explicit experiment, not a promotion.
...
//          floor gates on.
static const int V18A_QUALITY_FLOOR_M8 = 60;   // dead in this build — M8 is masked off

// ============================================================================
//  [VARIANT M2M4FB] CONFIG BLOCK — the only behavioural deltas vs v12.38
// ============================================================================
//  Mode mask. A masked mode still evaluates and still logs to the EVAL line
//  (so the diagnostics stay honest about what the signal engine saw), but it
//  can never be selected for an order. selMode indices: M1=0 M2=1 M3=2 M4=3
//  M5=4 M6=5 M7=6 M8=7.
static const bool C_VAR_ENABLE_M1 = false;
static const bool C_VAR_ENABLE_M2 = true;    // Delta Divergence Reversal
static const bool C_VAR_ENABLE_M3 = false;
static const bool C_VAR_ENABLE_M4 = true;    // Level Sweep Rejection
static const bool C_VAR_ENABLE_M6 = false;
static const bool C_VAR_ENABLE_M8 = false;   // Fade Engine
//  (M5 and M7 are already inactive in the v12.24+ signal-combination line.)

//  Fixed bracket. Dollars PER CONTRACT, converted to points at runtime with
//  PtVal (= CurrencyValuePerTick / TickSize), so the same numbers are correct
//  on NQ and MNQ. Set either to 0 to fall back to the production ATR bracket.
...
//  MAIN STUDY
// ============================================================================
SCSFExport scsf_IOF_NQ_Autopilot_M2M4_FixBrkt(SCStudyInterfaceRef sc)
{
    // ---- Subgraphs ----
    ...
    if(sc.SetDefaults){
        sc.GraphName="IOF NQ Autopilot [M2+M4 QF40 FixBrkt 500/500 - VARIANT]";
        sc.StudyDescription="IOF NQ Pure Orderflow — VARIANT BUILD, NOT the validated live config. "
            "Modes: M2 + M4 only (M1/M3/M6/M8 hard-masked). Quality floor 40 all modes. "
            "Fixed bracket: $500 stop / $500 target per contract (NQ 25pt, MNQ 250pt), 1:1 R:R — "
            "no T1 partial, no breakeven shift, no ATR trail, no early-scratch. "
            "Max 3 trades/day, daily loss $1600 (= 3 stops; required for MT=3 to be reachable). "
            "Consec-loss 2 and the 15:00 late-entry gate still apply. "
            "RTH only (09:35 ET open, 15:55 flatten). 3k (3000) vol bars. CSV IOF_NQ_*.csv.";
        ...
        //
        //  ─── PHASE 1 — DEFAULTS BELOW  [VARIANT M2M4FB values] ──────────────
        //    Quantity           : 1
        //    Daily Loss $       : 1600        (= 3 x $500 fixed stop + headroom)
        //    Daily Profit $     : 0 (off)     (don't truncate right-tail days)
        //    Max Trades         : 3
        //    News Filter        : 1 (ON)
        //    Entry Order        : 2 (lmt+2t)
        //    Session Start      : 0 (RTH only)
        //    M1 VWAP Reclaim    : 0 (off)     — masked by C_VAR_ENABLE_M1
        //    Fade Engine (M8)   : 0 (off)     — masked by C_VAR_ENABLE_M8
        //
        //  WORST-CASE DAY under these defaults: 3 stops x $500 = $1,500 at 1
        //  contract, vs $500 in the production 1-trade/$800 config. That is 3x
        //  the daily tail risk against a $5,000 Apex trailing drawdown — the
        //  cushion drops from ~10 bad days to ~3. The consec-loss-2 rule caps
        //  most sequences at 2 stops ($1,000), but do not plan around it.
        //
        ...
        // [VARIANT M2M4FB] $1,600 = 3 x $500 stop + $100 slippage headroom.
        //   Sized to MAX_TRADES, not chosen for its own sake: the session-DD
        //   gate refuses new entries once drawdown > 0.8 * DAILY_LOSS, so at
        //   the production $800 the 2nd stop ($1,000 DD) would end the day and
        //   MAX_TRADES=3 could never bind. Lower this and you silently undo #3.
        IN_DAILY_LOSS.Name="Daily Loss $ (drives risk budget)"; IN_DAILY_LOSS.SetFloat(1600.f);
        IN_MAX_TRADES.Name="Max Trades/Day"; IN_MAX_TRADES.SetInt(3);  // [VARIANT M2M4FB] 1 -> 3 as requested
        ...
        IN_FADE_ENABLE.Name="Fade Engine (0=off)"; IN_FADE_ENABLE.SetInt(0);  // [VARIANT M2M4FB] M8 off — panel matches C_VAR_ENABLE_M8=false
        ...
        IN_M1_ENABLE.Name="Enable M1 VWAP Reclaim"; IN_M1_ENABLE.SetInt(0);   // [VARIANT M2M4FB] M1 off — panel matches C_VAR_ENABLE_M1=false
    ...
    // [VARIANT M2M4FB] Single source of truth for "this build runs a fixed $
    ...
    //   bypass the ATR bracket). Keeping one predicate means the two halves
    //   cannot disagree about which exit regime a live trade is under.
    ...
    if(!signalOnly){
        ...
        if(TradeState!=0&&Idx!=LastExitBar){
            ...
            // ================================================================
            //  [VARIANT M2M4FB] FIXED-BRACKET POSITION MANAGEMENT
            // ================================================================
            ...
            //  Leaving any of them live would move the stop or cut the target
            //  and the realised R would stop being the 1:1 that was requested.
            //
    ...
    //  SIGNAL COMBINATION
    // ============================================================================
    // [VARIANT M2M4FB] Mode mask. Applied AFTER the EVAL diagnostic above so
    //   the log still shows every mode the signal engine actually saw — only
    //   selection is suppressed. This is the single choke point: every masked
    //   mode's priority branch below becomes unreachable.
    if(!C_VAR_ENABLE_M1){ m1L=false; m1S=false; }
    if(!C_VAR_ENABLE_M2){ m2L=false; m2S=false; }
    if(!C_VAR_ENABLE_M3){ m3L=false; m3S=false; }
    if(!C_VAR_ENABLE_M4){ m4L=false; m4S=false; }
    if(!C_VAR_ENABLE_M6){ m6L=false; m6S=false; }
    if(!C_VAR_ENABLE_M8){ m8L=false; m8S=false; }
    ...
    // ---- [VARIANT M2M4FB] FIXED $ BRACKET ----------------------------------
    //  Overrides every mode-specific stop/target path below. Dollars -> points
    //  via PtVal so NQ and MNQ both land on the requested risk. T1 and T2 are
    //  set to the SAME price: there is one target, and the downstream exit
    //  manager is gated (see FIXBRKT blocks in the position-management section)
    //  so no partial, breakeven shift, trail or early-scratch can move it.
    ...
    // [VARIANT M2M4FB] fixBrkt short-circuits this second chain too — without
    //   the guard, M2/M4 (selMode 1/3) would fall through to the ATR `else`
    //   below and silently overwrite the fixed levels.
    if(fixBrkt){
    ...
    // [VARIANT M2M4FB] Skipped under the fixed bracket. The levels are already
    ...
    //   the fixed bracket exists to eliminate.
    ...
    if(LOG_LVL>=LOG_SIG){
        // [VARIANT M2M4FB] Report the bracket in DOLLARS as well as price, so a
        //   live log can be checked against the requested $500/$500 without
    ...
    // [VARIANT M2M4FB] The fixed bracket has ONE target for the whole position,
    //   at any quantity. Never split into T1/T2 legs here — T1Px == T2Px in this
    //   build, so a split would attach two orders at the same price and turn a
    //   1:1 trade into an unintended scale-out.
    if(fixBrkt){
