"""A/B: daily-VWAP directional filter on ALL modes + M2 QUAL_FLOOR=40.

User request: "filter all mode by daily vwap and m2 q40 and backtest."

Two levers, combined into one test arm:
  1. VWAP_FILTER_ALL=True  — every mode may only trade in the direction of the
     session VWAP (long > vwap, short < vwap). Applied after the regime gate.
  2. QUAL_FLOOR_M2=40      — lower M2's quality floor 50->40 (all other modes
     stay at 50). Note: M2 floor <50 has been FALSIFIED standalone at 25/30/40
     ([[project_m2_qf25_falsified]]); here it rides ON TOP of the VWAP filter,
     which is a different combined hypothesis — the VWAP alignment may be what
     rescues the extra M2 setups the floor admits.

Baseline = current production config (v12.37 early-scratch ON, MT=6/DL=800).
Both arms share it; only the two levers above flip in the test arm.

Cross-contract gate [[feedback_cross_contract_ab]]: the combined arm must AGREE
test_better on NQZ25 + NQM5 + NQH6 before it is a ship candidate.

Usage: python backtest_vwap_all_m2q40_ab.py [NQZ25|NQM5|NQH6]   (default: all)
"""
import csv
import math
import sys
...
SCENARIOS = {
    "baseline_live":       {**_PROD},
    "vwapAll_m2q40_test":  {**_PROD, "VWAP_FILTER_ALL": True, "QUAL_FLOOR_M2": 40},
}
BASELINE = "baseline_live"
TEST = "vwapAll_m2q40_test"
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_vwapAllM2q40_{tag}_{name}.csv"
    sys.argv = ["backtest.py", str(scid), str(out)]
    print(f"\n========== {tag} :: {name} ==========")
    print(f"  VWAP_FILTER_ALL={overrides.get('VWAP_FILTER_ALL')} "
          f"QUAL_FLOOR_M2={overrides.get('QUAL_FLOOR_M2')} QUAL_FLOOR=50")
    backtest.main()
    return out
...
def main():
    ...
    print("\n" + "=" * 84)
    print(" VWAP-FILTER-ALL + M2 QUAL_FLOOR=40  A/B SUMMARY (prod config)")
    print("=" * 84)
    print(f"  {'contract':8s} {'scenario':20s} {'n':>4s} {'WR%':>6s} {'PF':>6s} "
          f"{'Net':>10s} {'MaxDD':>9s} {'Sortino/d':>10s}")
    for tag, scn in results.items():
        for name, r in scn.items():
            print(f"  {tag:8s} {name:20s} {r['n']:>4} {r['wr']:>6.1f} {r['pf']:>6.2f} "
    ...
    print(" M2-only breakdown (what the floor+vwap combo unlocked):")
    print(f"  {'contract':8s} {'scenario':20s} {'M2_n':>5s} {'M2_WR%':>7s} {'M2_PF':>6s} {'M2_Net':>10s}")
    for tag, scn in results.items():
        for name, r in scn.items():
            m = r["m2"]
            print(f"  {tag:8s} {name:20s} {m['n']:>5} {m['wr']:>7.1f} {m['pf']:>6.2f} {m['net']:>+10,.0f}")
        print()
    ...
    if vs == {"test_better"}:
        ...
        print("\n test not better anywhere -- keep baseline (no VWAP filter, M2 floor 50)")
