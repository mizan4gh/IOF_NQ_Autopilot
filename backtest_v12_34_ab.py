"""A/B test: v12.32 baseline (continuation) vs v12.34 prior-bar dip+reclaim.

Cross-contract A/B convention applies (see [feedback_cross_contract_ab]) —
NQZ25 + NQM5 must AGREE fixed_better before shipping. Only flip cpp
sc.Input[16] IN_M1_PULLBACK to 2 in the live chartbook if both contracts
return fixed_better.

History:
  - v12.33 (mode=1, wick-reject) failed A/B 2026-05-22 on both contracts —
    over-filtered M1's reclaim continuation wins. See
    [project_v1233_m1_pullback_killed].
  - v12.34 (mode=2, dip+reclaim) tests a different shape: prior bar must
    have dipped TO VWAP and closed weak; current bar reclaims. Distinct
    SETUPs from baseline, not a strict filter on top.

Usage:
    python backtest_v12_34_ab.py            # NQZ25 + NQM5 default
    python backtest_v12_34_ab.py NQZ25      # single contract
"""
import csv
import sys
...
SCENARIOS = {
    ...
    "v12_34_dipreclaim": {**_APEX, "M1_PULLBACK": 2},
}
...
def print_contract(tag, results):
    print("\n" + "=" * 78)
    print(f"COMPARISON: v12.32 baseline vs v12.34 dip+reclaim  ({tag})")
    print("=" * 78)
    cols = list(results.keys())
    print(f"{'Metric':<22} " + " ".join(f"{c:>20s}" for c in cols))
    print("-" * 78)
    rows = [
        ("trades_taken",     lambda r: f"{r['n']:>20d}"),
        ("wins / losses",    lambda r: f"{r['wins']:>9d} / {r['losses']:<8d}"),
        ("win_rate",         lambda r: f"{r['wr']:>19.1f}%"),
        ("profit_factor",    lambda r: f"{r['pf']:>20.2f}"),
        ("total_pnl",        lambda r: f"${r['total']:>18.0f}"),
        ("expectancy",       lambda r: f"${r['expectancy']:>18.2f}"),
        ("max_drawdown",     lambda r: f"${r['max_dd']:>18.0f}"),
        ("m1_trades",        lambda r: f"{r['m1_n']:>20d}"),
        ("m1_win_rate",      lambda r: f"{r['m1_wr']:>19.1f}%"),
        ("m1_pnl",           lambda r: f"${r['m1_pnl']:>18.0f}"),
        ("setups_rm_gated",  lambda r: f"{r['rm_gated']:>20d}"),
    ]
    for label, fmt in rows:
        print(f"{label:<22} " + " ".join(fmt(results[c]) for c in cols))

    base = results.get("v12_32_baseline")
    fixed = results.get("v12_34_dipreclaim")
    if base and fixed:
        ...
        print("-" * 78)
        print(f"DELTA (v12.34 - v12.32):  trades={d_trades:+d}  pnl=${d_pnl:+.0f}")
...
def main():
    ...
    if len(all_results) >= 2:
        ...
        for tag, r in all_results.items():
            base = r["v12_32_baseline"]; fixed = r["v12_34_dipreclaim"]
            d_pnl = fixed["total"] - base["total"]
        ...
        dirs = [
            ("fixed_better" if all_results[t]["v12_34_dipreclaim"]["total"] >
                              all_results[t]["v12_32_baseline"]["total"]
             else "fixed_worse")
            for t in all_results
        ]
        if len(set(dirs)) == 1 and dirs[0] == "fixed_better":
            verdict = "AGREE fixed_better — safe to flip IN_M1_PULLBACK=2 live"
