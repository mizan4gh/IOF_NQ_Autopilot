"""
M4-ALONE, floor 40, fixed 75/125pt bracket — cross-contract NQ.

Same as backtest_nq_m4only_f50_fixbrkt.py but QUAL_FLOOR=40 (M4 uses *10
scaling => admits finalScore>=4; floor 50 required >=5 and fired ~nothing,
floor 30 fired score>=3). Tests the middle band: does score-4 M4 alone carry a
robust cross-contract edge with the fixed 75/125 bracket?

Prior (different config — full mode set / ATR exits): M4@40 test_worse on both
NQZ25 and NQM5 (project_m4_floor_40_failed, project_qf_profile_falsified).
"""
...
def run(label, fname):
    ...
    trades = bt.run()
    backtest.write_csv(trades, BASE / f"IOF_NQ_m4f40_{label}.csv")
    return trades
...
def main():
    ...
    print("\n" + "=" * 84)
    print(" M4-ONLY  floor 40  fixed 75/125pt bracket  (MT=1, 1 NQ lot, native 3000-vol)")
    print("=" * 84)
