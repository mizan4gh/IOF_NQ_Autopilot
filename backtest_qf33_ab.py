"""A/B: QUAL_FLOOR=33 vs live 50 — 3-contract gate.

Motivated by 2026-06-10 live no-fire day: 14 candidates armed, all killed by
the quality floor; best of day was M2 short quality 33 vs floor 50. Question:
does admitting the 33+ band add edge, or is the floor correctly rejecting it?

Prior art: floor 40 REJECTED ([[project_v1226_qualfloor_test]]), M2-only 25
FALSIFIED ([[project_m2_qf25_falsified]]). This is global 33 on the current
production config (v12.37 early-scratch ON in both arms).

Usage: python backtest_qf33_ab.py [NQZ25|NQM5|NQH6]   (default: all three)
"""
import csv
import math
import sys
...
SCENARIOS = {
    "qf50_live": {**_PROD, "QUAL_FLOOR": 50},
    "qf33_test": {**_PROD, "QUAL_FLOOR": 33},
}
BASELINE = "qf50_live"
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_qf33_{tag}_{name}.csv"
...
def main():
    ...
    print("\n" + "=" * 78)
    print(" QUAL_FLOOR=33 A/B SUMMARY (prod config, early-scratch ON)")
    print("=" * 78)
    ...
    print(" Verdict per contract (qf33 vs qf50):")
    verdicts = {}
    for tag, scn in results.items():
        b, t = scn[BASELINE], scn["qf33_test"]
