"""VWAP-touch engine + PULLBACK gate (+/- delta) — 3-contract sweep.

Refinement of the 18:00-anchored VWAP-touch engine
([[project_vwap_touch_engine_falsified]]): require a genuine PULLBACK into the
VWAP from the trend side (long only when price pulls back DOWN to a rising VWAP
from above; short only when it pulls UP to a falling VWAP from below), rejecting
momentum crosses through the VWAP. This targets the exact green/red pullback
setups the user circled on the live chart.

Sweeps SD width {0.25, 0.50} with and without delta>=3 confirmation.
Ship gate: net>0 AND PF>1 on all three [[feedback_cross_contract_ab]].

Usage: python backtest_vwap_touch_pullback_ab.py [NQZ25|NQM5|NQH6]   (default all)
"""
import csv
import math
import sys
...
_BASE = dict(
    ...
    VWAP_TOUCH_COOL=3,
    VWAP_TOUCH_PULLBACK=True,
)

SCENARIOS = {
    "pb_sd25":    {**_BASE, "VWAP_NEAR_SD": 0.25, "VWAP_TOUCH_DELTA_MIN": 0},
    "pb_sd50":    {**_BASE, "VWAP_NEAR_SD": 0.50, "VWAP_TOUCH_DELTA_MIN": 0},
    "pb_sd25_d3": {**_BASE, "VWAP_NEAR_SD": 0.25, "VWAP_TOUCH_DELTA_MIN": 3},
    "pb_sd50_d3": {**_BASE, "VWAP_NEAR_SD": 0.50, "VWAP_TOUCH_DELTA_MIN": 3},
}
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_vtpb_{tag}_{name}.csv"
    sys.argv = ["backtest.py", str(scid), str(out)]
    print(f"\n===== {tag} :: {name} (SD={overrides['VWAP_NEAR_SD']} "
          f"delta>={overrides['VWAP_TOUCH_DELTA_MIN']} pullback) =====")
    backtest.main()
    return out
...
def main():
    ...
    print("\n" + "=" * 88)
    print(" VWAP-TOUCH + PULLBACK gate (18:00 anchor) — 3-contract sweep")
    print("=" * 88)
    print(f"  {'contract':8s} {'scen':11s} {'n':>4s} {'L/S':>7s} {'WR%':>6s} {'PF':>6s} "
          f"{'Net':>10s} {'MaxDD':>9s} {'Sortino/d':>10s}")
    for tag, scn in results.items():
        for name, r in scn.items():
            ls = f"{r['n_long']}/{r['n_short']}"
            print(f"  {tag:8s} {name:11s} {r['n']:>4} {ls:>7s} {r['wr']:>6.1f} {r['pf']:>6.2f} "
    ...
    for name in SCENARIOS:
        ...
        print(f"  {name:11s} pooled {pooled:>+10,.0f}  -> {'PASS' if ok else 'FAIL'}")
