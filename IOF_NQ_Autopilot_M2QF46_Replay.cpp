...
SCDLLName("IOF_NQ_Autopilot_M2QF46_Replay")
// ============================================================================
// REPLAY-TEST VARIANT — NOT FOR LIVE TRADING.
// Identical to production v12.38 except V18A_QUALITY_FLOOR_M2 = 46 (M2-only
// floor drop, sel==1). Built to ground-truth the 2026-07-09 Python A/B
// displacement finding on NQU26 via Sierra bar replay + trade-sim fills.
// Verdict from backtest_m2_qf46_ab.py was DISAGREE / no-ship; this variant
// exists only to verify the live-contract arm. Delete after the replay.
// ============================================================================
...
// [REPLAY VARIANT] M2-only floor 46 (score>=7 band). Python A/B said this
// displaces higher-EV trades under MT=1; this constant exists to verify that
// on NQU26 with real cpp + trade-sim fills. Production keeps 50.
static const int V18A_QUALITY_FLOOR_M2 = 46;
...
//  MAIN STUDY
// ============================================================================
SCSFExport scsf_IOF_NQ_Autopilot_M2QF46_Replay(SCStudyInterfaceRef sc)
{
    // ---- Subgraphs ----
    ...
    if(sc.SetDefaults){
        sc.GraphName="IOF NQ Autopilot [M2QF46 REPLAY TEST - NOT LIVE]";
    ...
    if(!BannerShown && DIAG) {
        InitRunID();
        SCString m;
        m.Format("=== V18A v12.38-M2QF46-REPLAY-TEST LOAD sym=%s tick=%.4f tickval=$%.2f Cap=$%.0f DailyLoss=$%.0f DailyProf=$%.0f "
                 "MaxTr=%d FlatT=%d Qty=%d Live=%d Regime=%d Fade=%d News=%d AutoDis=%d M1=%d "
                 "RM_FLOOR=%.2f QUAL_FLOOR=%d(M2=46,M8=60) CD_trd=%d CD_loss=%d CD_stop=%d RunID=%llu ===",
    ...
    // [REPLAY VARIANT] M2 (selMode==1) lowered to 46 for the NQU26 ground-truth replay.
    int effFloor = (selMode==7) ? V18A_QUALITY_FLOOR_M8
                 : (selMode==1) ? V18A_QUALITY_FLOOR_M2
                 : V18A_QUALITY_FLOOR;
