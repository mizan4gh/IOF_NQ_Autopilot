"""
M4-ALONE, floor 60, fixed 75/125pt bracket — cross-contract NQ.

Floor 60 => M4 (finalScore*10) admits only finalScore>=6. Floor 50 (score>=5)
already fired just 2 trades across 4 contracts, so this is expected to be even
emptier — run to confirm the score-6 M4 population is effectively nil.
"""
...
def run(label, fname):
    ...
    trades = bt.run()
    backtest.write_csv(trades, BASE / f"IOF_NQ_m4f60_{label}.csv")
    return trades
...
def main():
    ...
    print("\n" + "=" * 84)
    print(" M4-ONLY  floor 60  fixed 75/125pt bracket  (MT=1, 1 NQ lot, native 3000-vol)")
    print("=" * 84)
