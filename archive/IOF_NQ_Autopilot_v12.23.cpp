// ============================================================================
//  IOF NQ — NQ Pure Orderflow Autopilot
//  Sierra Chart ACSIL Study
//
//  Version: v12.23 (Apr 2026; v12.20/21/22 + BAR_VOLUME auto-scaling)
//
//  CHANGES SINCE v11:
//    [v12.23b] Default Flatten HHMM aligned with Apex Trader Funding futures rule:
//             positions must be flat before 4:59 PM ET (see Apex support: trading day
//             through 4:59 PM ET). Default 1655 = 4:55 PM ET bar time — small buffer
//             before the hard cutoff; flatten runs when bar end >= input (volume bars
//             may not land on exact minutes). Adjust input if your account rules differ.
//    [v12.23] BAR_VOLUME auto-scaling (input slot 26, default 1000).
//             Original constants were calibrated at 3000 vol/bar. Running
//             on smaller bars (1k, 2k) without adjustment caused:
//               - cooldowns measured in BARS expired too quickly in wall
//                 time (10-bar after-loss = ~5min instead of ~15min)
//               - ATR shrunk proportionally but stop/target FLOORS stayed
//                 fixed at 20/32 pts, forcing R:R back toward 1.0
//               - VWAP slope 20-bar lookback shortened from ~1hr to ~20min
//             v12.23 introduces K = 3000/BAR_VOLUME and applies it to:
//               bar-count gates (x K):  after-trade, after-loss, post-stop,
//                                       vwap-mature, delta-mature, m5-cooldown,
//                                       spike-cooldown, vwap-slope-lookback
//               point floors (/ K):     stop floor, t1 floor
//               slope tolerance (/ K):  M4 vwap-trend gate tolerance
//               trade budget (x K):     MAX_TRADES/day
//             Default 1000 matches legacy v701 bar setup. Set to 3000 to
//             run at V18A's original calibration, 2000 for middle ground.
//
//    [v12.22] M4 VWAP-trend gate. Live sample 2026-01-02 10:12 BUY M4
//             at 25582.50 with score=5, divStr=4, delta=+526 — all the
//             intrabar filters loved it. But VWAP was ~150pt above price
//             and sloping down for 20+ bars (clear downtrend on chart).
//             Trade stopped at 99% MAE for -$800. M4 was buying a
//             counter-trend bounce in a trending-down market.
//
//             The code already computes sOKL/sOKS (VWAP slope 20-bar
//             lookback, ATR*0.02 tolerance) but applies it ONLY to M1.
//             v12.22 adds a parallel gate for M4 with a looser tolerance
//             (ATR*0.15, new constant C_M4_VWAP_TOL) to avoid over-
//             blocking in flat sessions. Rules:
//               - BUY M4 blocked if Close<VWAP AND VWAP slope < -tol
//               - SELL M4 blocked if Close>VWAP AND VWAP slope > +tol
//             Price above/below VWAP alone is not enough — slope must
//             also confirm trend direction. Logs as "VWAP trend gate".
//
//    [v12.21] Simple Mode (input slot 25, DEFAULT ON). Live sample
//             showed M4 did 7 of 8 trades and produced all 3 winners;
//             M8 did the 8th trade (one stop-out loss). M1, M2, M3,
//             M6, M7 never armed or never filled in 19 sessions.
//             Running every mode adds noise and priority-contention
//             (M5 armed 115 times, filled 0, crowded out M4 on shared
//             bars). Simple Mode forces m1..m3, m5..m8 to false before
//             selection, leaving a clean M4-only setup. Set input 25
//             to 0 to restore the full 8-mode engine.
//
//    [v12.20] FOUR trade-management fixes driven by 8-trade live sample
//             (37.5% WR, 0.78 PF, -$811 over 19 sessions, Mar 23-Apr 20).
//             Forensic: 5/5 losing trades went into profit first, 3 of
//             them reached 68-92% of TP1 distance before reversing and
//             stopping out at 98-99% of stop distance. Entry quality is
//             fine; trade management was leaking premium.
//
//             (a) R:R FIX. C_T1_ATR 1.25 -> 1.60 (R:R ~1.04 -> ~1.33).
//                 Prior defaults produced near-symmetric stop/target so
//                 break-even win rate was ~49%. Also widen T1 clamp
//                 floor/ceiling (25/50 -> 32/65 pts) so low-ATR bars
//                 don't walk TP1 back to stop-distance parity.
//
//             (b) NEAR-TARGET EXIT (single-lot). When absPos==1, if
//                 TradeMFE reaches >=70% of TP1 distance AND price has
//                 given back >=40% of MFE, flatten at market as FR_T1.
//                 Restores partial-profit capability that was structurally
//                 disabled when v12.18 forced qty=1. Uses existing
//                 PF_TradeMFE (already tracked every bar).
//
//             (c) CHOP TIME-STOP. If chopRegime>=1 and no T1 hit by
//                 EntryBar+6, flatten as FR_TIMEOUT. Live sample had
//                 losses hold 10 and 12 bars; winners all exited in 3-5.
//                 New FR_TIMEOUT reason code (11) + "TIMEOUT" CSV string.
//
//             (d) SELL-SIDE EXHAUSTION GATE. Skip SELL entries when
//                 Delta0 < -200 AND chopRegime>=1. Both live SELLs
//                 entered with large negative delta already printed.
//                 Narrow gate (delta threshold + chop only) to avoid
//                 blocking legitimate trend shorts.
//
...
// RTH flatten in ET (HHMM). Apex (futures): flat before 4:59 PM ET — default 1655 (4:55 PM)
// for a few minutes' buffer; first bar with BarHHMM >= this triggers flatten.
...
static const float C_T1_ATR         = 1.60f;  // [v12.20a] was 1.25f; R:R ~1.04 -> ~1.33
...
static const int   C_OPEN_COOL      = 15;
...
static const float C_M4_VWAP_TOL    = 0.15f;  // [v12.22] M4 VWAP slope tolerance (ATR fraction); loose vs M1's 0.02
...
static const float C_T1_FLOOR_PTS   = 32.0f;  // [v12.20a] was 25.0f
static const float C_T1_CEIL_PTS    = 65.0f;  // [v12.20a] was 50.0f
...
static const int FR_TIMEOUT      = 11;  // [v12.20c] chop time-stop exit
...
struct AdaptiveThresholds {
    ...
    float aggrBullish()   const { return calibrated ? FMin(0.65f,emaAggrPct+0.08f) : 0.55f; }
...
//  MAIN STUDY
// ============================================================================
SCSFExport scsf_IOF_NQ_Autopilot(SCStudyInterfaceRef sc)
{
    // ---- Subgraphs ----
    ...
    SCInputRef IN_SIMPLE_MODE=sc.Input[25];   // [v12.21] M4-only mode
    SCInputRef IN_BAR_VOLUME=sc.Input[26];   // [v12.23] chart bar volume (1000/2000/3000)

    if(sc.SetDefaults){
        sc.GraphName="IOF NQ v12.19";
        sc.StudyDescription="IOF NQ Pure Orderflow — v12.19 (Apr 2026). "
            "v12.15 fixes + optional V1-style hooks (confirmations / chop / R-Sharpe tighten). "
        ...
        IN_FLAT_TIME.Name="Flatten HHMM (ET; Apex: flat before 4:59pm — def 1655=4:55pm; bar end>=";
        IN_FLAT_TIME.SetInt(iof_unified::kDefaultFlattenHhmm);
        ...
        IN_ENTRY_ORD.Name="Entry (0=mkt,1=lmt,2=lmt+2t)"; IN_ENTRY_ORD.SetInt(1);
        ...
        IN_SIMPLE_MODE.Name="Simple Mode — M4 only (1=on recommended, 0=all modes)"; IN_SIMPLE_MODE.SetInt(1);  // [v12.21]
        IN_BAR_VOLUME.Name="Chart Bar Volume (auto-scales cooldowns/floors; 1000/2000/3000)";
        IN_BAR_VOLUME.SetInt(1000); IN_BAR_VOLUME.SetIntLimits(500, 5000);  // [v12.23]
        return;
    }
    ...
    const int SIMPLE_MODE=IN_SIMPLE_MODE.GetInt();  // [v12.21] M4-only
    // [v12.23] BAR_VOLUME scaling. Original constants were calibrated at 3000
    //          vol/bar. On smaller bars (1k, 2k), each bar captures less time
    //          and less price range; cooldowns (counted in BARS) need to grow
    //          so the wall-clock pause stays roughly constant, and point
    //          floors need to shrink so ATR-based stops/targets don't clamp
    //          to fixed values. K = 3000/BAR_VOLUME is the "bars per 3k-bar"
    //          multiplier. Applied as:
    //            - bar-count cooldowns, maturity gates, lookbacks: x K  (more bars)
    //            - point floors (STOP, T1): / K  (smaller price range)
    //            - VWAP slope tolerance: / K  (shorter lookback => tighter)
    //            - MAX_TRADES: x K  (denser signal flow)
    const int BAR_VOLUME_IN = IN_BAR_VOLUME.GetInt();
    const int BAR_VOLUME = IClamp(BAR_VOLUME_IN, 500, 5000);
    const float BAR_K = 3000.f / (float)BAR_VOLUME;
    // Scaled integer bar counts (round up so we don't under-shoot cooldowns)
    const int BC_AFTER_TRADE   = (int)ceilf(C_COOLDOWN_AFTER_TRADE * BAR_K);
    const int BC_AFTER_LOSS    = (int)ceilf(C_COOLDOWN_AFTER_LOSS  * BAR_K);
    const int BC_POST_STOP     = (int)ceilf(C_POST_STOP_COOLDOWN   * BAR_K);
    const int BC_VWAP_MATURE   = (int)ceilf(C_VWAP_MATURE          * BAR_K);
    const int BC_DELTA_MATURE  = (int)ceilf(C_DELTA_MATURE         * BAR_K);
    const int BC_M5_COOLDOWN   = (int)ceilf(C_M5_COOLDOWN          * BAR_K);
    const int BC_SPIKE_COOL    = (int)ceilf(C_SPIKE_COOL           * BAR_K);
    const int BC_VWAP_SLP_LB   = (int)ceilf(C_VWAP_SLP_LB          * BAR_K);
    // Scaled point floors (smaller bars => smaller ATR => smaller floors)
    const float BF_STOP_FLOOR  = C_STOP_FLOOR_PTS / BAR_K;
    const float BF_T1_FLOOR    = C_T1_FLOOR_PTS   / BAR_K;
    // Scaled VWAP tolerance (shorter bars => tighter slope threshold)
    const float BF_M4_VWAP_TOL = C_M4_VWAP_TOL    / BAR_K;
    // Scaled MAX_TRADES (denser signal flow on smaller bars)
    const int BC_MAX_TRADES    = IClamp((int)ceilf(MAX_TRADES * BAR_K), 1, 30);
    if(TICK<=0.f)return;
    ...
    if(PrevPosQty!=0&&CurQ==0&&LiveTradeDir!=0){
        ...
        else if(FlattenReason==FR_TIMEOUT){ExR="TIMEOUT"; FlattenReason=FR_NONE;}  // [v12.20c]
        else if(isL){
    ...
    if(NEWS_FILTER && pNews){
        ...
        if(pNews->spikeActive&&BC_SPIKE_COOL>0&&Idx<=pNews->spikeBar+BC_SPIKE_COOL){  // [v12.23] scaled
            goto TRADE_MGMT;
    ...
    if(!signalOnly){
        ...
        if(TradeState!=0&&Idx!=LastExitBar){
            ...
            // [v12.20c] Chop time-stop: flatten if held too long in chop
            //           without T1 hit. Live sample: losses held 10, 12 bars;
            //           winners all exited in 3-5. Long holds in
            //           chopRegime>=1 are asymmetrically bad.
            if(!T1Hit && pRegime && pRegime->chopRegime>=1 && Idx>=EntryBar+6){
                FlattenReason=FR_TIMEOUT;
                s_SCNewOrder em; em.OrderType=SCT_ORDERTYPE_MARKET;
                em.OrderQuantity=(int)FAbs((float)posQty);
                if(isL) sc.BuyExit(em); else sc.SellExit(em);
                TradeState=0; T1Hit=0; T1HitBar=-1; LastExitBar=Idx;
            ...
            // [v12.20b] Near-target exit for single-lot positions.
            //           If TradeMFE reached >=70% of TP1 distance AND
            //           current price has given back >=40% of MFE,
            //           flatten at market as FR_T1. Only for absPos==1
            //           since qty>=2 already has the partial-take leg.
            {
                int absPos_nt=(int)FAbs((float)posQty);
                if(!T1Hit && absPos_nt==1){
                    float t1Dist_nt = FAbs(T1Px - EntryPx);
                    float curFav_nt = isL ? (Close0 - EntryPx) : (EntryPx - Close0);
                    if(t1Dist_nt>0.f && TradeMFE >= t1Dist_nt*0.70f){
                        float giveback_nt = TradeMFE - curFav_nt;
                        if(giveback_nt >= TradeMFE*0.40f && curFav_nt > 0.f){
                            FlattenReason=FR_T1;
                            s_SCNewOrder nt_ex; nt_ex.OrderType=SCT_ORDERTYPE_MARKET;
                            nt_ex.OrderQuantity=absPos_nt;
                            if(isL) sc.BuyExit(nt_ex); else sc.SellExit(nt_ex);
                            TradeState=0; T1Hit=0; T1HitBar=-1; LastExitBar=Idx;
                            return;
                        }
                    }
        ...
        if(posQty!=0) return;
        if(Trades>=BC_MAX_TRADES) {  // [v12.23] scaled
        ...
        if(Idx==LastExitBar) return;
        {
            ...
            const int cdBars = (LastExitWasLoss == 1) ? BC_AFTER_LOSS : BC_AFTER_TRADE;  // [v12.23] scaled
    ...
    if(NEWS_FILTER&&pNews){
        ...
        if(pNews->spikeActive&&BC_SPIKE_COOL>0&&Idx<=pNews->spikeBar+BC_SPIKE_COOL) return;  // [v12.23] scaled
    ...
    const bool vwapOK=(BC_VWAP_MATURE<=0)||(VWAPBars>=BC_VWAP_MATURE);  // [v12.23] scaled
    const bool isNewBar=(Idx!=LastCalcBar);
    if(isNewBar) LastCalcBar=Idx;
    const bool deltaMature=(BC_DELTA_MATURE<=0)||(VWAPBars>=BC_DELTA_MATURE);  // [v12.23] scaled
    ...
    if(BC_VWAP_SLP_LB>0&&Idx>=BC_VWAP_SLP_LB){  // [v12.23] scaled
        float vp=SG_VWAP[Idx-BC_VWAP_SLP_LB];
        if(vp>0.f&&VWAP>0.f){
            vwapSlope=VWAP-vp;
            float tol=ATR*0.02f;
    ...
    if(isNewBar&&pImb&&Idx>=10&&ATR>0.f){
        ...
        if(aggR>=0.60f) imbStr++;
        if(aggR>=0.70f) imbStr++;
        int accept=0;
        ...
        if(imbStr>=3&&imbDir!=0){
            ...
            if(pImb->active&&pImb->startBar>=0&&Idx<=pImb->startBar+pImb->barCount+3)
                pImb->strength=FMax(0,pImb->strength-1);
            else pImb->reset();
        }
    }
    ...
    if(C_SWEEP_LB>0&&Idx>=C_SWEEP_LB&&ATR>0.f){
        ...
        // [v12.22] M4 VWAP-trend gate. Live sample (2026-01-02 10:12 BUY)
        //          bought a sweep 150pt BELOW a VWAP that was sloping down
        //          for 20+ bars. Stopped for -$800. M1 already has a slope
        //          filter (sOKL/sOKS) but M4 never used it. Block M4 entries
        //          when price and VWAP disagree with trade direction:
        //             - BUY M4 blocked if Close0<VWAP AND VWAP trending down
        //             - SELL M4 blocked if Close0>VWAP AND VWAP trending up
        //          Tolerance C_M4_VWAP_TOL (ATR*0.15) is looser than M1's
        //          ATR*0.02 to avoid over-blocking in flat/balanced sessions.
        if((m4L||m4S) && VWAP>0.f && BC_VWAP_SLP_LB>0 && Idx>=BC_VWAP_SLP_LB){  // [v12.23] scaled
            float vp_m4 = SG_VWAP[Idx-BC_VWAP_SLP_LB];
            if(vp_m4>0.f){
                float slope_m4 = VWAP - vp_m4;
                float tol_m4 = ATR * BF_M4_VWAP_TOL;  // [v12.23] scaled
                bool vwapDown = (slope_m4 < -tol_m4);
                bool vwapUp   = (slope_m4 >  tol_m4);
                if(m4L && Close0<VWAP && vwapDown){
                    m4L=false;
                    if(DIAG&&LOG_LVL>=LOG_SIG&&Idx!=LastBlockLogBar){
                        LastBlockLogBar=Idx;
                        SCString m; m.Format("[V18A] Skip M4 L - VWAP trend gate (Close<VWAP, slope=%.2f)", slope_m4);
                        sc.AddMessageToLog(m,0);
                    }
                }
                if(m4S && Close0>VWAP && vwapUp){
                    m4S=false;
                    if(DIAG&&LOG_LVL>=LOG_SIG&&Idx!=LastBlockLogBar){
                        LastBlockLogBar=Idx;
                        SCString m; m.Format("[V18A] Skip M4 S - VWAP trend gate (Close>VWAP, slope=%.2f)", slope_m4);
                        sc.AddMessageToLog(m,0);
                    }
                }
            }
    ...
    if(pTrap&&pTrap->valid&&pTrap->phase==3){
        bool m5CoolOK=(BC_M5_COOLDOWN<=0)||(LastM5Bar<0)||(Idx>LastM5Bar+BC_M5_COOLDOWN);  // [v12.23] scaled
        if(m5CoolOK){
    ...
    if(PrevImbDir!=0&&ATR>0.f){
        ...
        if(imbFading||imbDead){
            ...
            if(bPct>=0.55f&&corrDir) rvVerifyScore++;
    ...
    if(DIAG && LOG_LVL >= LOG_DBG && isNewBar){
        int bestSc = FMax(DL?scL:(LBL?lbScL:0), DS?scS:(LBS?lbScS:0));
    ...
    // [v12.21] SIMPLE MODE: live data (Mar-Apr 2026, 8 trades) showed
    //          M4 was the only profitable mode. Force-disable all others
    //          when SIMPLE_MODE=1 (default). Set input 25 to 0 to restore
    //          the full 8-mode selection.
    if(SIMPLE_MODE){
        m1L=m1S=false; m2L=m2S=false; m3L=m3S=false;
        m5L=m5S=false; m6L=m6S=false; m7L=m7S=false; m8L=m8S=false;
    }
    ...
    // [v12.20d] SELL-side exhaustion gate. Live SELLs 0-for-2 (-$1,470);
    //           both entered with Delta0 already strongly negative
    //           (-300, -234) in chopRegime>=1 -- shorting into the
    //           exhaustion low rather than the initiation.
    if(!selLong && Delta0 < -200 && pRegime && pRegime->chopRegime>=1){
        if(DIAG&&LOG_LVL>=LOG_SIG&&Idx!=LastBlockLogBar){
            LastBlockLogBar=Idx;
            SCString m; m.Format("[V18A] Skip M%d S - SELL exhaustion gate (Delta=%d chop=%d)",
                selMode+1, Delta0, pRegime->chopRegime);
            sc.AddMessageToLog(m,0);
        }
        return;
    }
    ...
    if(BC_POST_STOP>0&&LastStopBar>=0&&Idx<=LastStopBar+BC_POST_STOP){  // [v12.23] scaled
    ...
    if(selMode==5 && m6Stop!=0.f){
        ...
        stopDist = FClamp(stopDist, BF_STOP_FLOOR, C_STOP_CEIL_PTS);  // [v12.23] floor scaled
        float t1Dist = ATR * C_T1_ATR;
        t1Dist = FClamp(t1Dist, BF_T1_FLOOR, C_T1_CEIL_PTS);  // [v12.23] floor scaled
    ...
    float minStopDist = FMax(ATR*0.5f, BF_STOP_FLOOR);  // [v12.23] scaled
    if(selLong){
        if(stopPrice >= entryPrice - TICK) stopPrice = TFloor(entryPrice - minStopDist, TICK);
        if(tp1Price  <= entryPrice + TICK) tp1Price  = TCeil (entryPrice + FMax(ATR*1.0f, BF_T1_FLOOR), TICK);  // [v12.23]
        ...
        if(tp1Price  >= entryPrice - TICK) tp1Price  = TFloor(entryPrice - FMax(ATR*1.0f, BF_T1_FLOOR), TICK);  // [v12.23]
    ...
    if(LOG_LVL>=LOG_SIG){
        SCString m; m.Format("[V18A SETUP] %s %s Q=%d Sc=%d Ent=%.2f SL=%.2f T1=%.2f T2=%.2f Qty=%d RM=%.2f",
...
// ============================================================================
//  END OF IOF NQ — NQ Pure Orderflow Autopilot
// ============================================================================
