...
//
//  Version: v12.22 (Apr 2026; v12.20/21 + M4 VWAP-trend gate + M4/short fix)
//
//  CHANGES SINCE v11:
...
//
//             [v12.22b] SELL exhaustion gate (v12.20d) vs M4: that gate
//             skips SELL when Delta0 < -200 in chop. M4 SELL requires
//             bearish delta (DS) and often prints -200+ on the signal
//             bar — the gate vetoed valid M4 shorts. Exempt selMode==3
//             (M4); other modes keep the exhaustion filter.
//
...
SCSFExport scsf_IOF_NQ_Autopilot(SCStudyInterfaceRef sc)
{
    ...
    if(sc.SetDefaults){
        ...
        IN_LIVE.Name="Enable Auto Trading (1=live orders)"; IN_LIVE.SetInt(0);
        ...
        if(TradeState!=0&&Idx!=LastExitBar){
            ...
            if(!T1Hit&&Idx>EntryBar+3){
                ...
                if(sH){
                    ...
                    if(isL) sc.SellExit(em); else sc.BuyExit(em);
                ...
                if(!T1Hit && absPos_nt==1){
                    ...
                    if(t1Dist_nt>0.f && TradeMFE >= t1Dist_nt*0.70f){
                        ...
                        if(giveback_nt >= TradeMFE*0.40f && curFav_nt > 0.f){
                            ...
                            if(isL) sc.SellExit(nt_ex); else sc.BuyExit(nt_ex);
                            TradeState=0; T1Hit=0; T1HitBar=-1; LastExitBar=Idx;
                            return;
                        }
                    }
            ...
            if(!T1Hit){
                ...
                if(t1H){
                    ...
                    if(absPos>=2){
                        ...
                        t1x.OrderQuantity=q;
                        if(isL) sc.SellExit(t1x); else sc.BuyExit(t1x);
                    }
            ...
            if(T1Hit&&Idx>EntryBar+3){
                ...
                if(sH){
                    ...
                    s_SCNewOrder em; em.OrderType=SCT_ORDERTYPE_MARKET;
                    em.OrderQuantity=(int)FAbs((float)posQty);
                    if(isL3) sc.SellExit(em); else sc.BuyExit(em);
                    TradeState=0; T1Hit=0; T1HitBar=-1; LastExitBar=Idx;
                    return;
                }
            }
            if(Idx>EntryBar+2){
                ...
                if(t2H){
                    ...
                    if(isL4) sc.SellExit(t2x); else sc.BuyExit(t2x);
                    TradeState=0; T1Hit=0; T1HitBar=-1; LastExitBar=Idx;
                    return;
                }
            }
    ...
    if(ATR>0.f&&Idx>=C_CONSOL_LB&&vwapOK){
        // Build the consolidation box from prior bars only. Including the
        // current bar makes fresh breakout bars redefine rH/rL and blocks
        // legitimate M3 breakout signals on the trigger bar.
        float rH=sc.High[Idx-1], rL=sc.Low[Idx-1];
        for(int k=2;k<=C_CONSOL_LB&&Idx>=k;k++){
            if(sc.High[Idx-k]>rH) rH=sc.High[Idx-k];
            if(sc.Low[Idx-k]<rL) rL=sc.Low[Idx-k];
        }
        float rW=rH-rL;
    ...
    if(C_SWEEP_LB>0&&Idx>=C_SWEEP_LB&&ATR>0.f){
        ...
        if((m4L||m4S) && VWAP>0.f && C_VWAP_SLP_LB>0 && Idx>=C_VWAP_SLP_LB){
            float vp_m4 = SG_VWAP[Idx-C_VWAP_SLP_LB];
            if(vp_m4>0.f){
                float slope_m4 = VWAP - vp_m4;
                float tol_m4 = ATR * C_M4_VWAP_TOL;
    ...
    // [v12.22b] M4 (selMode==3) is exempt: M4 SELL arms on bearish delta;
    //           strong negative Delta0 is expected, not "late exhaustion".
    if(!selLong && selMode!=3 && Delta0 < -200 && pRegime && pRegime->chopRegime>=1){
        if(DIAG&&LOG_LVL>=LOG_SIG&&Idx!=LastBlockLogBar){
            LastBlockLogBar=Idx;
