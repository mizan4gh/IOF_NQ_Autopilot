...
//
//  Version: v12.21 (Apr 2026; v12.20 fixes + Simple Mode = M4 only by default)
//
//  CHANGES SINCE v11:
//    [v12.22] News blackout: always advance NewsLogSession/NewsLogWindow when
//             entering a calendar news window (not only when DIAG=1), so
//             dedup state is valid with diagnostics off. Simple Mode default
//             = 1 (M4 only); use 2 for M4+M8.
...
//             bars). Simple Mode forces competing modes off before
//             selection: 1 = M4 only; 2 = M4 + M8 only (M1 still off
//             via input 15); 0 = full 8-mode engine.
//
...
//            separation on 3000-volume bars.
//
...
SCSFExport scsf_IOF_NQ_Autopilot(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef IN_SIMPLE_MODE=sc.Input[25];   // [v12.21] 0=all 1=M4 2=M4+M8
    ...
    if(sc.SetDefaults){
        ...
        IN_SIMPLE_MODE.Name="Simple Mode (0=all, 1=M4 only, 2=M4+M8)"; IN_SIMPLE_MODE.SetInt(1); IN_SIMPLE_MODE.SetIntLimits(0, 2);  // M4-only default
        return;
    }
    ...
    const int SIMPLE_MODE=IN_SIMPLE_MODE.GetInt();  // 0=all 1=M4 2=M4+M8
    if(TICK<=0.f)return;
    ...
    if(NEWS_FILTER && pNews){
        if(pNews->isNewsWindow(BarHHMM)){
            ...
            // Dedup state must advance even when DIAG=0; otherwise NewsLog* never
            // tracks the active window and newWindow stays true every bar.
            if(newWindow){
    ...
    // [v12.21] SIMPLE MODE: reduce mode contention. 1 = M4 only; 2 = M4 + M8;
    //          0 = full selection (M1 still gated by M1_ENABLE).
    if(SIMPLE_MODE==1){
        m1L=m1S=false; m2L=m2S=false; m3L=m3S=false;
        m5L=m5S=false; m6L=m6S=false; m7L=m7S=false; m8L=m8S=false;
    }else if(SIMPLE_MODE==2){
        m1L=m1S=false; m2L=m2S=false; m3L=m3S=false;
        m5L=m5S=false; m6L=m6S=false; m7L=m7S=false;
    }
