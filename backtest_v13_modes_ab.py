"""A/B: v13 mode set (M1/M4/M6/M8) vs v12.37 full set (adds M2/M3) — 3-contract.

APPROXIMATION of the v13 rewrite ([[project_v13_rewrite]]). Captures v13's
biggest behavioral change — dropping M2 and M3 — by suppressing them via
DISABLE_MODES={1,2}. Both arms otherwise run the production v12.37 config
(early-scratch on, floors 50).

What this DOES model: the mode-set reduction to M1/M4/M6/M8.
What it does NOT model (so this is a floor, not a verdict, on v13):
  - v13's narrower M8 (drops trap/M7/iceberg fade types; backtest M8 is the
    balance-edge fade only, ~= v13 M8 type 1, missing trend-exhaustion).
  - v13's reduced confirmation score (drops S7/S8/S10/S11 sub-signals).
True v13 validation needs Sierra replay or a v13-faithful Python port.

Cross-contract gate: [[feedback_cross_contract_ab]].

Usage: python backtest_v13_modes_ab.py [NQZ25|NQM5|NQH6]   (default: all three)
"""
import csv
import math
import sys
...
SCENARIOS = {
    "v1237_all":  {**_PROD, "DISABLE_MODES": set()},
    "v13_modes":  {**_PROD, "DISABLE_MODES": {1, 2}},   # drop M2, M3
}
BASELINE = "v1237_all"
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_v13modes_{tag}_{name}.csv"
    sys.argv = ["backtest.py", str(scid), str(out)]
    print(f"\n========== {tag} :: {name} ==========")
    print(f"  DISABLE_MODES = {overrides['DISABLE_MODES'] or '{}'}")
    backtest.main()
    return out
...
def main():
    ...
    print("\n" + "=" * 80)
    print(" v13 MODE SET (M1/M4/M6/M8) vs v12.37 FULL — approximation, prod config")
    print("=" * 80)
    ...
    print(" Verdict per contract (v13_modes vs v1237_all):")
    verdicts = {}
    for tag, scn in results.items():
        b, t = scn[BASELINE], scn["v13_modes"]
    ...
    if vs == {"test_better"}:
        print("\n ALL AGREE test_better -- dropping M2/M3 helps cross-contract")
    elif vs == {"tied"}:
        print("\n TIED -- M2/M3 contribute ~nothing under prod caps")
    elif "test_better" in vs:
        print("\n DISAGREE -- mixed; M2/M3 removal not a clear win")
    else:
        print("\n test_worse somewhere -- M2/M3 removal costs on >=1 contract")
