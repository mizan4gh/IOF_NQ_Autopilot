"""A/B: M6-only QUAL_FLOOR=40 vs live 50 — 3-contract gate.

Retest of the M6-only floor lever at 30 (prior 25 FALSIFIED 2026-06-04:
NQZ25 -$2,830 / NQM5 -$1,595 / NQH6 +$8,860 disagreed,
[[project_m2_qf25_falsified]]). Floor lowered for M6 (sel==5) ONLY; all
...
Usage: python backtest_m6_qual40_ab.py [NQZ25|NQM5|NQH6]   (default: all three)
"""
import csv
import math
import sys
...
SCENARIOS = {
    "qf50_live":  {**_PROD, "QUAL_FLOOR_M6": None},
    "m6qf40_test": {**_PROD, "QUAL_FLOOR_M6": 40},
}
BASELINE = "qf50_live"
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_m6qf40_{tag}_{name}.csv"
    sys.argv = ["backtest.py", str(scid), str(out)]
    print(f"\n========== {tag} :: {name} ==========")
    print(f"  QUAL_FLOOR=50 QUAL_FLOOR_M6={overrides['QUAL_FLOOR_M6']}")
    backtest.main()
    return out
...
def summarize(csv_path):
    ...
    if n == 0:
        return dict(n=0, total=0.0, pf=0.0, wr=0.0, max_dd=0.0,
                    ...
                    m6=dict(n=0, net=0.0, wr=0.0, pf=0.0))
    ...
    m6_pnls = [t["pnl"] for t in trades if t["mode"] == "M6"]
    m6_w = [p for p in m6_pnls if p > 0]
    m6_l = [p for p in m6_pnls if p < 0]
    m6 = dict(n=len(m6_pnls), net=sum(m6_pnls),
              wr=100 * len(m6_w) / len(m6_pnls) if m6_pnls else 0.0,
              pf=(sum(m6_w) / abs(sum(m6_l))) if m6_l else float("inf"))
...
def main():
    ...
    print("\n" + "=" * 78)
    print(" M6-ONLY QUAL_FLOOR=40 A/B SUMMARY (prod config, early-scratch ON)")
    print("=" * 78)
    ...
    print(" M6-only breakdown (what the lever unlocked):")
    print(f"  {'contract':8s} {'scenario':12s} {'M6_n':>5s} {'M6_WR%':>7s} {'M6_PF':>6s} {'M6_Net':>10s}")
    for tag, scn in results.items():
        for name, r in scn.items():
            m = r["m6"]
    ...
    print(" Verdict per contract (m6qf40 vs qf50):")
    verdicts = {}
    for tag, scn in results.items():
        b, t = scn[BASELINE], scn["m6qf40_test"]
