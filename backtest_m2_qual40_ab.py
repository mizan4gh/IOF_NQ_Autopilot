"""A/B: M2-only QUAL_FLOOR=40 vs live 50 — 3-contract gate.
...
Usage: python backtest_m2_qual40_ab.py [NQZ25|NQM5|NQH6]   (default: all three)
"""
import csv
import math
import sys
...
SCENARIOS = {
    ...
    "m2qf40_test": {**_PROD, "QUAL_FLOOR_M2": 40},
}
BASELINE = "qf50_live"
...
def run_scenario(name, overrides, scid, tag):
    ...
    out = BASE / f"IOF_NQ_m2qf40_{tag}_{name}.csv"
...
def main():
    ...
    print("\n" + "=" * 78)
    print(" M2-ONLY QUAL_FLOOR=40 A/B SUMMARY (prod config, early-scratch ON)")
    print("=" * 78)
    ...
    print(" Verdict per contract (m2qf40 vs qf50):")
    verdicts = {}
    for tag, scn in results.items():
        b, t = scn[BASELINE], scn["m2qf40_test"]
